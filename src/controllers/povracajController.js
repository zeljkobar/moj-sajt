...
exports.getStatistikeByFirma = async (req, res) => {
  ...
  try {
    const [statistike] = await executeQuery(
      `SELECT 
         COUNT(DISTINCT p.id) as ukupno_pozajmica,
         COUNT(DISTINCT CASE WHEN p.status = 'aktivna' THEN p.id END) as aktivne_pozajmice,
         COUNT(DISTINCT CASE WHEN p.status = 'delimicno_vracena' THEN p.id END) as delimicno_vracene,
         COUNT(DISTINCT CASE WHEN p.status = 'potpuno_vracena' THEN p.id END) as potpuno_vracene,
         ...
         COUNT(pp.id) as ukupno_povracaja
       FROM pozajmnice p
