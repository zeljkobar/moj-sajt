...
async function debugConnection() {
  ...
  try {
    ...
    try {
      const [hostResult] = await connection.execute('SELECT @@hostname as hostname, @@port as port');
    ...
    try {
      ...
      const myProcess = processResult.find(p => p.Id === userResult[0].connection_id);
      if (myProcess) {
    ...
    try {
      ...
      // Test korisnici table specifically
      const [korisniciTest] = await connection.execute('SELECT COUNT(*) as count FROM korisnici');
      console.log('✅ Korisnici table accessible, rows:', korisniciTest[0].count);
    ...
    console.log('4. Check if user exists: SELECT user, host FROM mysql.user WHERE user = "zeljko"');
    console.log('5. Grant privileges: GRANT ALL PRIVILEGES ON summasum_.* TO "zeljko"@"%" IDENTIFIED BY "password"');
  }
}
