...
function createJPRUrl(firmaData, context = 'pregled') {
  ...
  if (!firmaData || !firmaData.id) {
    ...
    return '/shared/jpr-korica.html';
  }

  const url = `/shared/jpr-korica.html?firmaId=${firmaData.id}&context=${context}`;
...
function openJPRForRadnik(firmaData, radnikData) {
  ...
  if (!firmaData || !firmaData.id) {
    ...
    window.open('/shared/jpr-korica.html', '_blank');
    setTimeout(() => {
      window.open('/shared/jpr-dodatak-b.html', '_blank');
    }, 100);
    return;
  }
  ...
  if (
    ...
    window.open(
      `/shared/jpr-korica.html?firmaId=${firmaData.id}&context=radnik`,
      '_blank'
    );
    setTimeout(() => {
      window.open(
        `/shared/jpr-dodatak-b.html?firmaId=${firmaData.id}&context=radnik`,
        '_blank'
      );
    }, 100);
    return;
  ...
  const koricaUrl = `/shared/jpr-korica.html?firmaId=${
    firmaData.id
  ...
  const dodatakUrl = `/shared/jpr-dodatak-b.html?firmaId=${
    firmaData.id
  }&context=radnik&radnikIme=${encodeURIComponent(
    radnikData.ime
  )}&radnikPrezime=${encodeURIComponent(radnikData.prezime)}&radnikJmbg=${
...
function createJPRUrlForRadnik(firmaData, radnikData) {
  ...
  const url = `/shared/jpr-korica.html?firmaId=${
    firmaData.id
  }&context=radnik&radnikIme=${encodeURIComponent(
    radnikData.ime
  )}&radnikPrezime=${encodeURIComponent(radnikData.prezime)}&radnikJmbg=${
...
function openJPRForOdjavu(firmaData, radnikData) {
  ...
  if (
    ...
    window.open(
      `/shared/jpr-korica.html?firmaId=${firmaData.id}&context=odjava`,
      '_blank'
    );
    setTimeout(() => {
      window.open(
        `/shared/jpr-dodatak-b.html?firmaId=${firmaData.id}&context=odjava`,
        '_blank'
      );
    }, 100);
    return;
...
async function generisiUgovorModal() {
  ...
  try {
    ...
    if (radnik.vrsta_ugovora === 'ugovor_o_dopunskom_radu') {
      ugovorUrl = `/shared/ugovor-o-dopunskom-radu.html?radnikId=${radnikId}&firmaId=${currentFirmaId}`;
    } else if (radnik.vrsta_ugovora === 'ugovor_o_djelu') {
      ugovorUrl = `/shared/ugovor-o-djelu.html?radnikId=${radnikId}&firmaId=${currentFirmaId}`;
    } else if (radnik.vrsta_ugovora === 'ugovor_o_pozajmnici') {
      ugovorUrl = `/shared/ugovor-o-zajmu-novca.html?radnikId=${radnikId}&firmaId=${currentFirmaId}`;
    } else {
      // Default - ugovor o radu (ili bilo koja druga vrsta)
      ugovorUrl = `/shared/ugovor-o-radu.html?radnikId=${radnikId}&firmaId=${currentFirmaId}`;
    }
    ...
    window.open(
      `/shared/ugovor-o-radu.html?radnikId=${radnikId}&firmaId=${currentFirmaId}`,
      '_blank'
    );
  }
}
...
function potvrdiPotvrdaModal(radnikId, firmaId) {
  ...
  const url = `/shared/potvrda-zaposlenja.html?radnikId=${radnikId}&firmaId=${firmaId}&razlog=${encodeURIComponent(
    razlog
  )}`;
...
function editCurrentFirma() {
  ...
  window.location.href = `/shared/edit-firmu.html?pib=${currentFirmaPib}`;
}
...
function generateOdlukaRaspored() {
  ...
  window.open(
    `/shared/odluka-raspored-radnog-vremena.html?${params.toString()}`,
    '_blank'
  );
}
...
function potvrdiPorodiljskoModal() {
  ...
  const url = `/shared/resenje-porodiljsko-odsustvo.html?radnikId=${radnikId}&firmaId=${currentFirmaId}&datumPocetka=${datumPocetka}&datumDonosenja=${datumDonosenja}`;
  window.open(url, '_blank');
...
function kreirajCRPSZahtjev() {
  ...
  window.location.href = `/shared/crps-zahtjev.html?${params.toString()}`;
