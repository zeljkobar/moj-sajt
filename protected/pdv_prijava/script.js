...
document.getElementById("xmlButton").addEventListener("click", function () {
  ...
  const selectedFirma = firmeData.find((f) => f.pib === firma);
  if (!selectedFirma) {
    alert("Podaci o firmi nisu pronađeni");
    return;
  }

  exportXML(selectedFirma);
});

function exportXML(firma) {
  // Kreiraj XML strukturu za PDV prijavu
...
<PDVPrijava>
  ...
  <IzlazniPDV>
    <OporeziviPromet21>${
      document.getElementById("oporezivi_promet_21").value || 0
    }</OporeziviPromet21>
    <IzlazniPDV21>${
      document.getElementById("izlazni_pdv_21").value || 0
    }</IzlazniPDV21>
    <OporeziviPromet15>${
      document.getElementById("oporezivi_promet_15").value || 0
    }</OporeziviPromet15>
    <IzlazniPDV15>${
      document.getElementById("izlazni_pdv_15").value || 0
    }</IzlazniPDV15>
    <OporeziviPromet7>${
      document.getElementById("oporezivi_promet_7").value || 0
    }</OporeziviPromet7>
    <IzlazniPDV7>${
      document.getElementById("izlazni_pdv_7").value || 0
    }</IzlazniPDV7>
    <UkupanIzlazniPDV>${
      document.getElementById("ukupan_izlazni_pdv").value || 0
    }</UkupanIzlazniPDV>
  </IzlazniPDV>
  <UlazniPDV>
    <UlazniPDVDomaci>${
      document.getElementById("ulazni_pdv_domaci_promet").value || 0
    }</UlazniPDVDomaci>
    <PDVUvoz>${document.getElementById("pdv_uvoz").value || 0}</PDVUvoz>
    <UkupanUlazniPDV>${
      document.getElementById("ukupan_ulazni_pdv_pretporez").value || 0
    }</UkupanUlazniPDV>
  </UlazniPDV>
  <Rezultat>
    <DospjeliPDV>${
      document.getElementById("dospjeli_pdv").value || 0
    }</DospjeliPDV>
    <PDVKredit>${document.getElementById("pdv_kredit").value || 0}</PDVKredit>
  </Rezultat>
</PDVPrijava>`;
  ...
  a.href = url;
  a.download = `pdv_prijava_${firma.pib}_${new Date()
    .toISOString()
    .slice(0, 10)}.xml`;
